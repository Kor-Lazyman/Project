    ...
    def delivery(self, item_id, customer, product_inventory, daily_events):
        ...
        while True:
            ...
            if product_inventory.current_level <customer.order_history[-1]:
                ...
                self.num_shortages =self.num_shortages+ abs(product_inventory.current_level -  customer.order_history[-1])
...
# 무작위로 정수값을 뽑는 함수 정의
    def random_integer_from_exponential(self):
        ...
        while True:
            # 지수 분포로 난수 생성
            x = np.random.exponential(scale=1 / self.mean)
            random_integer = int(x)
            if random_integer <= 8:
    ...
    def order(self, sales, product_inventory, daily_events):
        ...
        while True:
            if self.cont==0:
                ...
                order_size =self.random_integer_from_exponential()
            ...
            # THIS WILL BE A RANDOM VARIABLE
                if sales.num_shortages>0:
                    order_size = self.order_size+sales.num_shortages
