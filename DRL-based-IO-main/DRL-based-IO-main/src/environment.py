...
class Inventory:
    ...
    def _cal_holding_cost(self, daily_events):
        if self.current_level-self.updated_inven>0:
            self.holding_cost = (self.current_level-self.updated_inven) * self.unit_holding_cost * \
...
class Provider:
    ...
    def deliver(self, demand_size, inventory, daily_events,time):
        # Lead time
...
class Procurement:
    ...
    def order(self, provider, inventory,daily_events,time,action):
            ...
            daily_events.append(
                    f"{self.env.now}: Placed an order for {action} units of {I[self.item_id]['NAME']}")
            print(f"{self.env.now}: Placed an order for {action} units of {I[self.item_id]['NAME']}")
            self.env.process(provider.deliver(
                   action , inventory, daily_events,time))
            self._cal_procurement_cost(action, daily_events)
...
class Production:
    ...
    def _cal_processing_cost(self, processing_time, daily_events):
        processing_cost = self.unit_processing_cost
        ...
        while True:
            ...
            if shortage_check==True and already_stop==False:
                ...
                # Processing time has elapsed
                    ...
                    print("Process_over:",self.process_id)
                    print(f"{self.env.now}: A unit of {self.output['NAME']} has been produced")
                    daily_events.append(
...
class Sales:
    ...
    def _cal_selling_cost(self, delivery_size, daily_events):
        self.daily_selling_cost = self.unit_delivery_cost * delivery_size+self.loss_cost
        daily_events.append(
...
# 랜덤한 수요 생성
    ...
    def order(self, sales, product_inventory, daily_events):
        ...
        while True:
            if self.cont==0:
                yield self.env.timeout(0)
                self.order_size=self.random_integer_from_exponential()
            ...
            # THIS WILL BE A RANDOM VARIABLE
                if sales.num_shortages>0:
                    self.order_size =self.random_integer_from_exponential()+sales.num_shortages
                    self.order_history.append(self.order_size)
                    daily_events.append(
             f"{self.env.now}: The customer has placed an order for {self.order_size} units of {I[self.item_id]['NAME']}")
                    daily_events.append(
