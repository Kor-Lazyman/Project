...
class MetaLearner:
    def __init__(
        ...
        **config,
        ...
        self.buffers = MultiTaskBuffer(
            ...
            # [num of adapatation for train] + [validation]
            num_episodes=(self.num_adapt_epochs + 1),
        ...
        self.result_path = os.path.join(
            "results", save_exp_name, save_file_name)
    ...
    def meta_update(self) -> Dict[str, float]:
        ...
        gradient = torch.autograd.grad(
            loss_before, self.agent.policy.parameters(), retain_graph=True)
        gradient = self.agent.flat_grad(gradient)
        Hvp = self.agent.hessian_vector_product(
            kl_before, self.agent.policy.parameters())

        search_dir = self.agent.conjugate_gradient(Hvp, gradient)
        descent_step = self.agent.compute_descent_step(
            Hvp, search_dir, self.max_kl)
        loss_before.detach_()
        ...
        for i in range(self.backtrack_iters):
            ...
            print(
                f"{i}-Backtracks | Loss {loss_after:.4f} < Loss_old {loss_before:.4f} : ", end="")
            print(
                f"{is_improved} | KL {kl_after:.4f} <= maxKL {self.max_kl:.4f} : {is_constrained}")

            if is_improved and is_constrained:
                print(
                    f"Update meta-policy through {i+1} backtracking line search step(s)")
                break
    ...
    def meta_train(self) -> None:
        ...
        for iteration in range(self.num_iterations):
            ...
            indices = np.random.randint(
                len(self.train_tasks), size=self.meta_batch_size)
    ...
    def visualize_within_tensorboard(self, results_summary: Dict[str, Any], iteration: int) -> None:
        ...
        self.writer.add_scalar(
            "train/loss_after", results_summary["loss_after"], iteration)
        self.writer.add_scalar(
            "train/kl_after", results_summary["kl_after"], iteration)
        self.writer.add_scalar("train/policy_entropy",
                               results_summary["policy_entropy"], iteration)
        ...
        self.writer.add_scalar(
            "time/total_time", results_summary["total_time"], iteration)
        self.writer.add_scalar("time/time_per_iter",
                               results_summary["time_per_iter"], iteration)
    ...
    def meta_test(
        ...
        for task in range(len(self.test_tasks)):
            ...
            batch_after_grad = self.buffers.get_trajs(
                task, self.num_adapt_epochs)
        ...
        results_summary["return_before_grad"] = sum(
            returns_before_grad) / len(self.test_tasks)
        results_summary["return_after_grad"] = sum(
            returns_after_grad) / len(self.test_tasks)
        if self.env_name == "vel":
            results_summary["run_cost_before_grad"] = run_cost_before_grad / \
                len(self.test_tasks)
            results_summary["run_cost_after_grad"] = run_cost_after_grad / \
                len(self.test_tasks)
        ...
        if self.is_early_stopping:
            ckpt_path = os.path.join(
                self.result_path, "checkpoint_" + str(iteration) + ".pt")
